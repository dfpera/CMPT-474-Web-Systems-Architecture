...
has_pended_id = False
next_opnum = 1
has_next_opnum = False
msg_pended = {}

class ID_Backend():
	def __init__(self, id, response, status): #mark: status
		self.id = id;
			...
			if not has_stored_id:
				...
				for pended in ID_Pended:
					if pended['opnum'] == next_opnum:
						...
						msg_pended = pended
						break
		#elif len(ID_Pended) == 0 or (not has_next_opnum):
		...
		if msg_in:
			...
			for pended in ID_Pended:
				if pended['opnum'] == msg_opnum: 
					print (pended['opnum'] == msg_opnum)
					# msg_response = stored.get_response()
					has_pended_id = True
					break
			if not has_pended_id:
				if msg_opnum > next_opnum:
					ID_Pended.append(body) # mark ID_Backend(...)
					q_in.delete_message(msg_in)
				elif msg_opnum == next_opnum:
					for stored in ID_Stored:
						if stored.get_id() == msg_id: 
							msg_response = stored.get_response()
							response.staturs = stored.ge_status()
